use crate::{error::CliError, gen::name::generate_player_name};
...
impl Pointercrate {
    pub async fn register_player(&mut self, nation_pool: &Vec<String>) -> Result<i32, CliError> {
        ...
        let mut player = Player::by_id(player.id, &mut connection).await?;
        player
            .set_nationality(
